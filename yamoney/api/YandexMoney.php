...
class YandexMoney {
    ...
    public function requestPaymentP2P($accessToken, $to, $amount, $comment = null, $message = null, $test_payment = false, $test_card = 'available', $test_result='success') {
        $paramArray['pattern_id'] = 'p2p';
        $paramArray['to'] = $to;
        $paramArray['amount'] = $amount;
        $paramArray['comment'] = $comment;
        $paramArray['message'] = $message;
        if((bool)$test_payment == true){
            $paramArray['test_payment'] = 'true';
            $paramArray['test_card'] = $test_card;
            $paramArray['test_result'] = $test_result;
        }
        $params = http_build_query($paramArray);
    ...
    public function processPaymentByWallet($accessToken, $requestId,$test_payment = false, $test_card = 'available', $test_result='success') {
    ...
    public function requestPaymentShop($accessToken, $shopParams, $test_payment = false, $test_card = 'available', $test_result='success') {
        if((bool)$test_payment == true){
            $shopParams['test_payment'] = 'true';
            $shopParams['test_card'] = $test_card;
            $shopParams['test_result'] = $test_result;
        }
    ...
    public function processPaymentByCard($accessToken, $requestId, $csc, $test_payment = false, $test_card = 'available', $test_result='success') {
        $paramArray['request_id'] = $requestId;
